...
def evaluate_turn(
    ...
    *,
    ...
    ----------
    hand
        Concealed-hand tile strings INCLUDING any just-drawn tile when
        making a discard decision. 13 tiles = no-discard analysis;
        14 tiles = full discard analysis. Red fives carry '*'.
    visible_tiles
        All tile faces the player can see *besides* their own concealed
        hand (the function adds the hand internally). Typically:
        own discards + opponents' discards + opponents' called melds +
        dora indicators.
    melds
        Convenience: extra meld tile groups to merge into visibility.
    dora_indicators
    ...
    if len(hand) > 14:
        raise ValueError(f"hand has {len(hand)} tiles, expected 13 or 14")
    ...
    if len(hand) == 14:
        per_discard = calculate_discard_ukeire(
            merged_hand, remaining, calculate_minimum_shanten, base_shanten=shanten_min
        )
        ukeire_values = [r.value for r in per_discard]
        best_idx = evaluate_best_discard(ukeire_values, dora=primary_dora)
        recommended = _format_normal(best_idx) if best_idx >= 0 else None

        if best_idx >= 0:
            current_ukeire = ukeire_values[best_idx]
